...
public:
    Sub1Node() : Node("sub1_node")
    {
        ...
        timer1=this->create_wall_timer(chrono::milliseconds(1000), bind(&Sub1Node::callback4, this));
        timer2=this->create_wall_timer(chrono::milliseconds(2000), bind(&Sub1Node::callback2, this));
    }
private:
    void callback(const turtlesim::msg::Pose::SharedPtr data1){
        x1=data1->x;
        y1=data1->y;
        theta1=data1->theta;
        RCLCPP_INFO(this->get_logger(), "%f, %f, %f",x1, y1, theta1);
    }
    void callback3(const turtlesim::msg::Pose::SharedPtr data2){
        x2=data2->x;
        y2=data2->y;
        theta2=data2->theta;
